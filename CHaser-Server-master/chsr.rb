...
def diffmap (ar1, ar2)
	...
	if result.size < 2
		...
		return result.sort{|a,b| b[2]<=>1[2]}
	end
end
...
#sql = 'INSERT INTO log VALUES(?, "ready", "*", "{cool:?,hot:?}", "[]", ?);'
#sql_log.execute(sql, @turn, @score[0], @score[1], @initmap)
str = {:turn => @turn,:player => "ready", :code => "*" , :score => {:cool => @score[0], :hot => @score[1] } , :diff => [],:map => @initmap}.to_json
send_log(str)
...
@turn_max.times do |turn|
  ...
  2.times do |i|
    ...
    #sql = 'INSERT INTO log VALUES(-1, "ready", "*", "{cool:?,hot:?}", ?, ?);'
    #@diff_map = diffmap(@logmap,@initmap)
    #sql_log.execute(sql, @score[0], @score[1], @diff_map, @logmap)
    str = {:turn => -1,:player => "ready" , :code => "*" , :score => {:cool => @score[0], :hot => @score[1] } , :diff => diffmap(@logmap,@initmap),:map => @logmap}.to_json
    send_log(str)
    @logmap_prev = @logmap
	end
    ...
    #sql = 'INSERT INTO log VALUES(?, "ready", ?, "{cool:?,hot:?}", "[]", ?);'
    #sql_log.execute(sql, @turn, @code0, @score[0], @score[1], @logmap)
    str = {:turn =>  @turn,:player => ["cool","hot"][i], :code => @code0 , :score => {:cool => @score[0], :hot => @score[1] } ,:diff => diffmap(@logmap,@logmap_prev),:map => @logmap}.to_json
    send_log(str)
    @logmap_prev = @logmap
...
#sql = 'INSERT INTO log VALUES(?, "end", ?, "[cool_win,hot_win,draw][winner]"", "{cool:?,hot:?}", "[]", ?)';
#sql_log.execute(sql, @turn, @code0, @score[0], @score[1], @logmap)
str = {:turn => @turn,:player => "end", :code => ["cool_win","hot_win","draw"][winner] , :score => {:cool => @score[0], :hot => @score[1] } , :diff => [],:map => @logmap}.to_json
send_log(str)
